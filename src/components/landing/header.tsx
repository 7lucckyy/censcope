...
export default function Header() {
  ...
  const isOpague = React.useMemo(() => pathname !== "/" || top > 20, [top]);
  ...
  return (
    ...
    >
      ...
      <Link
        href="/"
        className="bg-logo bg-contain w-20 md:w-44 h-14 md:h-20 bg-left md:bg-center bg-no-repeat"
      />

      <nav
        onClick={() => setOpen(false)}
        className={`fixed md:relative flex flex-col md:flex-row font-bold md:font-normal pt-40 md:pt-0 top-0 left-0 w-screen h-screen md:w-auto md:h-auto bg-white bg-opacity-80 backdrop-blur md:backdrop-blur-none md:bg-transparent md:flex items-center transition-all gap-10 md:gap-16 text-3xl md:text-lg capitalize md:pointer-events-auto md:opacity-100 ${
      ...
      >
        donate{" "}
        <FaArrowRight className=" text-white group-hover:scale-125 transition-all" />
      </Link>
